...
def header_html(email=None):
    ...
    return "<div class='topbar'><div class='logo'>MASTERPICK<span>AI</span><span class='badge'>V10.7 FIXED</span></div>" + nav + "</div>"
...
def home():
    ...
    if not email:
        html += "<div class='hero'><h1>Real Games On SportyBet</h1><p style='color:#8aa0c5;font-size:13px'>Kenya removed - Over 1.5 only</p><br><a href='/signup' class='btn btn-primary'>Start Free</a><br><br><a href='/login' class='btn btn-dark'>Login</a></div></body></html>"
        return html
    ...
    if fixtures:
        all_text = "MASTERPICKAI - SportyBet Verified - Over 1.5:\n\n"
        for f in fixtures[:4]:
    ...
    if len(fixtures) == 0:
        html += "<div class='card'><div class='league'>NO SPORTYBET GAMES THIS HOUR</div><div style='font-size:13px;margin-top:6px;color:#8aa0c5'>International break - Check 7PM MLS/Brazil. Kenya filtered.</div></div>"
    else:
        html += "<h3 style='margin:14px 4px'>FREE 2 Games @2.80 - Over 1.5</h3>"
        for f in fixtures[:2]:
            share_txt = "MASTERPICK FREE: " + f["match"] + " - Over 1.5 @" + f["odd"] + " - SportyBet OK"
        ...
        if user["plan"] == "pro" and user["status"] == "active":
            ...
            html += "<div class='card' style='text-align:center'><div class='match'>PRO LOCKED @7.50</div><div style='color:#8aa0c5;font-size:13px'>Over 1.5 - All SportyBet</div><br><a href='/plans' class='btn btn-primary'>View Plans</a></div>"
    html += "<br><a href='/plans' class='btn btn-dark'>Plans</a> <a href='/admin' style='display:block;text-align:center;color:#4a5f85;font-size:12px;margin-top:12px;text-decoration:none'>Admin</a></div></body></html>"
    return html
...
def login():
    if request.method == "POST":
        ...
        if em in USERS and USERS[em]["pass"] == pw:
            session["email"] = em
            return redirect("/")
    html = "<html><head>" + STYLE + "</head><body><div class='login-wrap'><h2>Login</h2><form method='post'><input class='input' name='email' placeholder='Email' required><input class='input' name='pass' type='password' placeholder='Password' required><br><br><button class='btn btn-primary'>Login</button></form></div></body></html>"
    return html
...
def signup():
    if request.method == "POST":
        ...
        session["email"] = em
        return redirect("/")
    html = "<html><head>" + STYLE + "</head><body><div class='login-wrap'><h2>Join Free</h2><form method='post'><input class='input' name='email' placeholder='Email' required><input class='input' name='pass' type='password' placeholder='Password' required><br><br><button class='btn btn-primary'>Create</button></form></div></body></html>"
    return html
