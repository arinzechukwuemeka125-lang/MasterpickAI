...
def init():
 ...
 db.commit()
 q=db.execute("SELECT * FROM users WHERE email=?",(ADMIN,)).fetchone()
 if not q:
  far=(datetime.now(WAT)+timedelta(days=3650)).isoformat()
  pw=hashlib.sha256("admin123".encode()).hexdigest()
  now=datetime.now(WAT).isoformat()
  db.execute("INSERT INTO users VALUES (NULL,?,?,?,?,?,?,?)",(ADMIN,pw,now,far,1,1,far))
  db.commit()
init()
@app.route("/")
def home(): return "<h1>LIVE</h1><a href=/login>Login</a>"
...
def login():
 if request.method=="POST":
  ...
  if not u:
   ...
   far=(datetime.now(WAT)+timedelta(days=3650)).isoformat()
   ex=far if is_ad else te
   now=datetime.now(WAT).isoformat()
   db.execute("INSERT INTO users VALUES (NULL,?,?,?,?,?,?,?)",(e,pw,now,te,is_ad,is_ad,ex))
   db.commit()
   session["user"]=e
   return redirect("/")
  if u["password"]==pw:
