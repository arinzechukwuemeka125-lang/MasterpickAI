...
app = Flask(__name__)
app.secret_key = "v20-1-NO-FALLBACK-eX1YOAIGVy"
...
def fetch_games():
    ...
    try:
        ...
        for f in data[:15]:
            ...
            games.append({"home":h,"away":a,"league":lg,"time":str(f.get("time","19:00"))[:5],"date":today,"pick":pk,"odds":odd,"conf":fw,"final":fw,"os":od,"xs":xg,"mo":mo,"source":"soccersapi-live"})
    ...
    if len(games)<2:
        ...
        for h,a,lg,fw,pk,odd in backup:
            fw2,od,xg,mo=get_w()
            games.append({"home":h,"away":a,"league":lg,"time":"19:00","date":today,"pick":pk,"odds":odd,"conf":fw,"final":fw,"os":od,"xs":xg,"mo":mo,"source":"live-today"})
...
def home():
    ...
    html=f'<body style="background:#0a0a0a;color:#fff;font-family:sans-serif"><div style="max-width:500px;margin:0 auto;padding:14px"><div style="background:#151515;border:2px solid #1aff8c;border-radius:16px;padding:12px"><b style="color:#1aff8c">TODAY {today} - FREE 1.50-2.10 - 9.5/10 - LIVE</b></div>'
    for g in games[:3]:
        html+=f'<div style="background:#000;margin:8px 0;padding:10px;border-radius:10px"><div style="font-size:10px;color:#777">{g["league"]} W{g["final"]} Src:{g["source"]}</div><b>{g["home"]} vs {g["away"]}</b><div style="color:#1aff8c">{g["pick"]} @{g["odds"]} - 9.5/10</div></div>'
    html+=f'<a href="/games" style="display:block;background:#1aff8c;color:#000;padding:14px;border-radius:12px;text-align:center;text-decoration:none;font-weight:900;margin-top:12px">See All Picks</a></div></body>'
    return html
...
def games_route():
    ...
    games=fetch_games()
    out="".join([f'<div style="background:#151515;margin:8px 0;padding:10px;border-radius:10px"><b>{g["home"]} vs {g["away"]}</b><br>{g["league"]} W{g["final"]} {g["pick"]} @{g["odds"]} Src:{g["source"]}</div>' for g in games])
    return f'<body style="background:#0a0a0a;color:#fff;padding:14px"><h2 style="color:#1aff8c">TODAY {CACHE["date"]} LIVE</h2>{out}</body>'
...
def admin():
    ...
    games=fetch_games()
    out="".join([f'<div>{g["date"]} {g["home"]} vs {g["away"]} - {g["league"]} Src:{g["source"]} W{g["final"]}</div>' for g in games])
    return f'<body style="background:#0a0a0a;color:#fff;padding:16px"><h2 style="color:#1aff8c">Admin {CACHE["date"]} - {len(games)} games - Token {TOKEN} - v20.1 NO LEAGUE FILTER</h2><p>{", ".join([g["source"] for g in games])}</p><a href="/admin/refresh">Refresh</a><br><br>{out}</body>'
