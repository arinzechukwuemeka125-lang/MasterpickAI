...
app = Flask(__name__)
app.secret_key = "masterpick_v83_secure_final"
...
def init_db():
    db=get_db()
    db.execute("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, email TEXT UNIQUE, password TEXT, created_at TEXT, trial_end TEXT, is_pro INT, is_admin INT)")
    db.commit()
    if not db.execute("SELECT * FROM users WHERE email=?",(ADMIN_EMAIL,)).fetchone():
        now=get_wat_now()
        db.execute("INSERT INTO users VALUES (NULL,?,?,?,?,?,1,1)",(ADMIN_EMAIL, hashlib.sha256("admin123".encode()).hexdigest(), now.isoformat(), (now+timedelta(days=3650)).isoformat()))
        db.commit()
...
def get_games():
    seed = get_wat_date_str(); random.seed(seed)
    free = [{"sport":"⚽ FOOTBALL","match":random.choice(REAL_FOOTBALL),"pick":"Home Win or Draw","odd":"1.38","conf":"93%"},{"sport":"⚽ FOOTBALL","match":random.choice(REAL_FOOTBALL),"pick":"Over 1.5 Goals","odd":"1.48","conf":"91%"}]
    pro = [{"sport":"⚽ FOOTBALL","match":random.choice(REAL_FOOTBALL),"pick":"Over 0.5","odd":"1.25"},{"sport":"🏀 BASKETBALL","match":random.choice(REAL_BASKET),"pick":"Over 158.5","odd":"1.42"},{"sport":"🏐 VOLLEYBALL","match":random.choice(REAL_VOLLEY),"pick":"Over 3.5 Sets","odd":"1.35"},{"sport":"🏓 TABLE TENNIS","match":random.choice(REAL_TTENNIS),"pick":"Over 3.5 Games","odd":"1.38"},{"sport":"⚽ FOOTBALL","match":random.choice(REAL_FOOTBALL),"pick":"1X","odd":"1.32"},{"sport":"🏀 BASKETBALL","match":random.choice(REAL_BASKET),"pick":"Home +5.5","odd":"1.40"}]
    return free, pro, seed, get_wat_now().strftime("%I:%M %p WAT - %d %b %Y")

def is_logged_in(): return "email" in session
def is_admin_user(): return session.get("email")==ADMIN_EMAIL
def is_pro_user():
    if is_admin_user(): return True
    db=get_db(); u=db.execute("SELECT * FROM users WHERE email=?",(session.get("email"),)).fetchone()
    if not u: return False
    try: return u["is_pro"]==1 or datetime.fromisoformat(u["trial_end"]) > get_wat_now()
    except: return False
...
<h2 style="color:#00ff88;text-align:center">💰 MasterpickAI</h2><p style="text-align:center;color:#888">{{msg}}</p>
<form method="POST">
...
<center><h2 style="color:#00ff88">💰 MasterpickAI V8.3 SECURE</h2><small style="color:#00ff88">{{wat}}</small><br><small style="color:#888">Seed: {{date}} - Auto 12AM WAT</small><br><small style="color:#888">Logged: {{email}} | <a href="/logout" style="color:#ff4444">Logout</a></small></center>
...
def signup():
    if request.method=="POST":
        email=request.form["email"].lower().strip(); pwd=hashlib.sha256(request.form["password"].encode()).hexdigest()
        db=get_db()
        ...
        now=get_wat_now(); db.execute("INSERT INTO users VALUES (NULL,?,?,?,?,0,0)",(email,pwd,now.isoformat(),(now+timedelta(days=1)).isoformat())); db.commit()
        session["email"]=email; return redirect("/")
...
def login():
    if request.method=="POST":
        ...
        if u and u["password"]==pwd: session["email"]=email; return redirect("/")
...
@app.route("/admin")
def admin_page():
    if not is_admin_user(): return "⛔ 403 FORBIDDEN - ADMIN ONLY - IP LOGGED",403
    ...
    for u in users: html+=f"<tr><td>{u['email']}</td><td>{u['is_pro']}</td><td>{u['is_admin']}</td></tr>"
    return html+"</table>"
