...
app = Flask(__name__)
app.secret_key = "v10_9_admin_history_final"
API_KEY = "8623f52e5c8224c49f7bb676d1f68665"

USERS = {
    "arinzechukwuemeka125@gmail.com": {"pass": "admin123", "plan": "pro", "status": "active", "expiry": datetime.now() + timedelta(days=365), "is_admin": True, "pending": None, "joined": "2026-09-01"}
}
...
ALLOW = ["brazil","saudi","mls","major league","premier league","la liga","serie a","bundesliga","ligue 1","eredivisie","primeira","super lig","champions","europa","conference","world cup","qualification","argentina","mexico","japan","usa","club friendly"]
...
def get_fixtures():
    ...
    try:
        ...
        for day_offset in [0,1]: # Today + Tomorrow so you always have games
...
def header_html(email=None):
    if email:
        nav = f"<div style='font-size:11px'><span style='background:#162032;padding:6px 10px;border-radius:20px'>{email[:18]}</span> <a href='/logout' style='color:#6b7fa3;margin-left:8px;text-decoration:none'>Logout</a></div>"
    else:
    ...
    return f"<div class='topbar'><div class='logo'>MASTERPICK<span>AI</span><span class='badge'>V10.9</span></div>{nav}</div>"
...
def home():
    ...
    if not email:
        html += "<div class='hero'><h1>Real Games Found On SportyBet</h1><p style='color:#8aa0c5;font-size:13px'>Kenya filtered - Over 1.5 only @1.40 - Today + Tomorrow</p><br><a href='/signup' class='btn btn-primary'>Create Free Account</a><br><br><a href='/login' class='btn btn-dark'>Login</a><br><br><a href='/history' class='btn btn-dark'>View Win/Loss History</a></div></body></html>"
        return html
    html += "<div style='padding:16px'>"
    html += f"<div style='display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px'><a href='/history' class='btn-wa' style='background:#162032;border:1px solid #23324f'>History WIN/LOSS</a><a href='/plans' class='btn-wa' style='background:#162032;border:1px solid #23324f'>Plans</a><a href='/admin' class='btn-wa' style='background:#162032;border:1px solid #23324f'>Admin</a></div>"
    if user and user["status"] == "pending":
        html += f"<div class='card' style='border-color:orange'><div class='league' style='color:orange'>PENDING APPROVAL</div><div class='match'>You paid N{user['pending']} - Wait admin approval</div></div>"
    html += f"<div style='margin:10px 0'><span class='stat'><b>{len(fixtures)}</b><br><span class='league'>Games Today+Tomorrow</span></span><span class='stat'><b>2.80</b><br><span class='league'>Free</span></span><span class='stat'><b>7.50</b><br><span class='league'>Pro</span></span></div>"
    if not fixtures:
        html += "<div class='card'><div class='league'>NO SPORTYBET GAMES - International Break</div><div style='font-size:13px;color:#8aa0c5;margin-top:6px'>EPL is on break. Games return full tomorrow. System checks Today+Tomorrow.</div></div>"
    else:
        html += "<h3 style='margin:14px 4px'>FREE 2 Games @2.80 - Over 1.5</h3>"
        for f in fixtures[:2]:
            wa = wa_link(f"FREE: {f['match']} - {f['tip']} @{f['odd']} - {f['league']} - {f['date']} {f['time']}")
            html += f"<div class='card'><div class='league'>{f['league']} - {f['date']} {f['time']} - SPORTY OK</div><div class='match'>{f['match']}</div><div class='tipbox'><div>{f['tip']}</div><div class='odd'>{f['odd']}</div></div><div style='margin-top:10px'><a href='{wa}' target='_blank' class='btn-wa'>WhatsApp Share</a></div></div>"
        html += "<h3 style='margin:18px 4px 8px'>PRO 6 Games @7.50</h3>"
        if user["plan"] == "pro" and user["status"] == "active":
            for f in fixtures[:6]:
                wa = wa_link(f"PRO: {f['match']} Over 1.5 @{f['odd']}")
            ...
            html += "<div class='card' style='text-align:center'><div class='match'>PRO LOCKED @7.50</div><div style='color:#8aa0c5;font-size:13px'>Pay to unlock 6 games Over 1.5</div><br><a href='/plans' class='btn btn-primary'>View Plans N1000-N15000</a></div>"
    html += "</div></body></html>"
    return html
...
def history_page():
    email = session.get("email")
    html = f"<html><head>{STYLE}</head><body>{header_html(email)}<div style='padding:16px;max-width:600px;margin:0 auto'><h2>Win / Loss History</h2><p style='color:#8aa0c5;font-size:13px'>Free @2.80 vs Pro @7.50 - Last 7 days</p>"
    for h in HISTORY:
...
@app.route("/plans")
def plans():
    html = f"<html><head>{STYLE}</head><body>{header_html(session.get('email'))}<div style='padding:16px;max-width:500px;margin:0 auto'><h2>Plans - Over 1.5 Only - SportyBet</h2><p style='color:#8aa0c5'>Opay 09079789177 - Arinze Peter</p>"
    for price, days in [("1000","3 Days"),("2000","7 Days"),("5000","15 Days"),("10000","25 Days"),("15000","30 Days")]:
        html += f"<div class='card'><div style='display:flex;justify-content:space-between'><div><div class='match'>N{price} - {days}</div><div class='league'>Over 1.5 @7.50</div></div><a href='/subscribe/{price}' class='btn btn-primary' style='width:auto'>I Paid</a></div></div>"
...
def admin_page():
    ...
    if not email or not USERS.get(email, {}).get("is_admin"):
        return f"<html><head>{STYLE}</head><body>{header_html(email)}<div style='padding:16px'>Admin only - login arinzechukwuemeka125@gmail.com / admin123<br><br><a href='/login' class='btn btn-primary'>Login Admin</a></div></body></html>"
    total = len(USERS)
    ...
    html = f"<html><head>{STYLE}</head><body>{header_html(email)}<div style='padding:16px'><h2>Admin Dashboard V10.9</h2><div style='margin:12px 0'><span class='stat'><b>{total}</b><br><span class='league'>Total Users</span></span><span class='stat'><b>{free}</b><br><span class='league'>Free</span></span><span class='stat'><b>{pro}</b><br><span class='league'>Pro Active</span></span><span class='stat'><b>{pending}</b><br><span class='league'>Pending</span></span></div>"
    if pending>0:
        html += "<h3 style='margin-top:20px'>Pending Payments</h3>"
        for em, u in USERS.items():
            if u["status"]=="pending":
                html += f"<div class='card' style='border-color:orange'><div class='match'>{em}</div><div class='league'>Paid N{u['pending']} - Joined {u.get('joined','-')}</div><br><a href='/admin/approve/{em}' class='btn btn-primary' style='width:auto;display:inline-block'>APPROVE</a> <a href='/admin/reject/{em}' class='btn btn-dark' style='width:auto;display:inline-block'>REJECT</a></div>"
    html += "<h3 style='margin-top:20px'>All Users</h3>"
    for em, u in USERS.items():
        html += f"<div class='card'><div style='display:flex;justify-content:space-between'><div><div class='match' style='font-size:13px'>{em}</div><div class='league'>{u['plan']} - {u['status']} - Joined {u.get('joined','-')}</div></div><div class='league'>{u.get('expiry','')}</div></div></div>"
    html += "<br><a href='/' class='btn btn-dark'>Home</a> <a href='/history' class='btn btn-dark'>History</a></div></body></html>"
    return html
...
def approve(email):
    if email in USERS:
        ...
        USERS[email]["expiry"] = datetime.now() + timedelta(days=30)
...
def signup():
    ...
    if request.method == "POST":
        ...
        pw = request.form["pass"]
        if em in USERS:
            err = "Email exists - login"
        else:
            USERS[em] = {"pass": pw, "plan": "free", "status": "active", "expiry": None, "is_admin": False, "pending": None, "joined": datetime.now().strftime("%Y-%m-%d")}
            session["email"] = em
            return redirect("/")
    html = f"<html><head>{STYLE}</head><body><div class='topbar'><div class='logo'>MASTERPICK<span>AI</span></div></div><div class='login-wrap'><h2>Create Free Account</h2><p style='color:#8aa0c5;font-size:13px'>2 Free Games @2.80 - SportyBet Over 1.5</p><br>"
    if err:
        html += f"<div class='card' style='border-color:red;color:#ff6b6b'>{err}</div>"
    html += "<form method='post'><input class='input' name='email' placeholder='Email' required><input class='input' name='pass' type='password' placeholder='Password' required><br><br><button class='btn btn-primary'>Create Free Account</button></form><br><p style='text-align:center;color:#8aa0c5;font-size:14px'>Have account? <a href='/login' style='color:#00ff88;text-decoration:none;font-weight:800'>Login Here</a></p><br><a href='/' class='btn btn-dark'>Back to Home</a></div></body></html>"
    return html
